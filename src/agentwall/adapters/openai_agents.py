"""OpenAI Agents SDK adapter -- AST-based parser for OpenAI Agents code."""
...
# OpenAI Agents SDK may use vector stores internally -- detect same class names.
...
class _OpenAIAgentsVisitor(ast.NodeVisitor):
    """AST visitor for a single Python file -- extracts OpenAI Agents SDK patterns."""
    ...
    # -- Assignments & calls: Agent(...), Runner.run(...), vector stores -----
    ...
    def visit_Assign(self, node: ast.Assign) -> None:
        if isinstance(node.value, ast.Call):
            ...
            self._check_runner_call(node.value)
            self._check_handoff_call(node.value)
        self.generic_visit(node)
    ...
    def _check_runner_call(self, call: ast.Call) -> None:
        """Detect Runner.run(agent, ...) calls."""
        func_name = _get_attr_chain(call.func)
        if func_name == "Runner.run":
            self.runner_count += 1

    def _check_handoff_call(self, call: ast.Call) -> None:
        """Detect handoff() calls (agent delegation)."""
...
def _get_attr_chain(node: ast.expr) -> str | None:
    """Return dotted name like 'Runner.run' from an ast.Attribute node."""
...
class OpenAIAgentsAdapter:
    """Parse an OpenAI Agents SDK project directory into an AgentSpec."""
    ...
    def parse(self, target: Path) -> AgentSpec:
        ...
        for py_file in py_files:
            ...
            visitor = _OpenAIAgentsVisitor(source_file=py_file)
            visitor.visit(tree)
            all_tools.extend(visitor.tools)
            all_memory.extend(visitor.memory_configs)
            total_agents += visitor.agent_count
            total_runners += visitor.runner_count
            total_handoffs += visitor.handoff_count
        ...
        if total_runners:
            metadata["runner_count"] = total_runners
        if total_handoffs:
            metadata["handoff_count"] = total_handoffs
