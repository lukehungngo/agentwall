...
class TestModuleLayoutDetection:
    ...
    def test_chromadb_module_no_metadata(self, tmp_path: Path) -> None:
        """No pyproject.toml, but chromadb/ package exists → True."""
        pkg = tmp_path / "chromadb"
        pkg.mkdir()
        (pkg / "__init__.py").touch()
        assert is_self_library_project(tmp_path) is True

    def test_src_layout_langchain(self, tmp_path: Path) -> None:
        """src/langchain_community/ package exists → True."""
